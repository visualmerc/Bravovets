...
(function ($) {
                    ...
                    loader = function () {
                        ...
                        $.ajax({
                            ...
                            if (json.result) {
                                ...
                                // rc: convert the UTC time to local

                                var theOffset = new Date().getTimezoneOffset() * 60000;

                                $.each(events, function (index, element) {
                                    var offset = -(new Date().getTimezoneOffset() / 60);

                                        var hours = parseInt(element.display_start);
                                        hours += offset;
                                        if (hours < 0) {
                                            hours = 24 + hours;
                                        }

                                        var localTimeString = (("0" + hours).slice(-2) + ":" + "00");
                                        element.display_start = localTimeString;

                                    element.start -= theOffset;
                                    element.end -= theOffset;
                                }
                                );
                            }
                        });
    ...
    Calendar.prototype._update_modal = function () {
        var self = this;
        $('a[data-event-id]', this.context).socialPostDialog({ "dialogType": "socialcal" }).remove();
        $('a[data-event-id]', this.context).socialPostDialog({ "dialogType": "socialcal" });

        //$('a[data-event-id]', this.context).unbind('click');


        //$('a[data-event-id]', this.context).click(function(event) {            
        //    event.stopPropagation();
        //    alert("a[data-event-id]:click");
        //});       
    };
    ...
    Calendar.prototype._update_month_year = function () {
        ...
        $('.cal-month-day')
            .on('mouseenter', function () {
                ...
                //backhere
                // $(this).find("a").socialPostDialog({ "dialogType": "socialcal" });
            })
            .on('mouseleave', function () {
                downbox.hide();
                // $(this).find("a").socialPostDialog({ "dialogType": "socialcal" }).remove();

            })
            .on('click', function (event) {
                var slidedown = true;
                // alert(".cal-month-day:click");
                ...
                showEventsList(event, downbox, slider, self, slidedown);
            });
        ...
        downbox.click(function (event) {
            showEventsList(event, $(this), slider, self, true);
        });
    };
    ...
    function showEventsList(event, that, slider, self, slidedown) {
        //magic here
        event.stopPropagation();
        ...
        // Add a class to style the currently selected day
        $('.cal-month-box .active').removeClass('active');
        ...
        $('a.event-item').mouseenter(function () {
            //    alert("a.event-item:mouseenter");
            // $(this).socialPostDialog({ "dialogType": "socialcal" });
            //   $('a[data-event-id="' + $(this).data('event-id') + '"]').closest('.cal-cell1').addClass('day-highlight dh-' + $(this).data('event-class'));
        });
        $('a.event-item').mouseleave(function () {
            //   alert("a.event-item:mouseleave");
            //$(this).socialPostDialog({ "dialogType": "socialcal" }).remove();
            //     $('div.cal-cell1').removeClass('day-highlight dh-' + $(this).data('event-class'));
        });
